...
function updateStats(){const a=getArena(P.tr);const totalBattles=P.wins+P.losses;const winRate=totalBattles>0?Math.round((P.wins/totalBattles)*100):0;const avgCrowns=totalBattles>0?Math.round(P.crowns/totalBattles*10)/10:0;const stats=[{i:'🏆',v:P.tr.toLocaleString(),l:'Trophies'},{i:'🏟️',v:a.name,l:'Arena'},{i:'✅',v:P.wins,l:'Wins'},{i:'❌',v:P.losses,l:'Losses'},{i:'👑',v:P.crowns,l:'Crowns'},{i:'🔥',v:P.maxStr,l:'Best Streak'},{i:'⚔️',v:totalBattles.toString(),l:'Total Battles'},{i:'📈',v:winRate+'%',l:'Win Rate'},{i:'🏅',v:avgCrowns,l:'Avg Crowns'},{i:'💰',v:P.gold.toLocaleString(),l:'Gold'},{i:'💎',v:P.gems.toLocaleString(),l:'Gems'},{i:'⭐',v:(P.starPoints||0).toLocaleString(),l:'Star Points'},{i:'💵',v:'$'+(P.cashBalance||0).toFixed(2),l:'Cash Balance'},{i:'🎴',v:P.unlocked.length+'/'+CARDS.length,l:'Cards'},{i:'🏷️',v:(P.discount||0)+'%',l:'Discount'}];document.getElementById('statsGrid').innerHTML=stats.map(s=>`<div class="stat-card"><div class="stat-icon">${s.i}</div><div class="stat-value">${s.v}</div><div class="stat-label">${s.l}</div></div>`).join('');const starEl=document.getElementById('dispStarPoints');if(starEl)starEl.textContent=(P.starPoints||0).toLocaleString();}
